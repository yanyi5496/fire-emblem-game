...
func initialize(query_service: Node) -> void:
	battle_query = query_service
...
func _get_move_cost(pos: Vector2i, tile_map: TileMap) -> int:
	...
	if tile_data:
		...
		if cost != null:
			return cost
	var terrain_data := _get_runtime_terrain_data(pos)
	if terrain_data.get("walkable", true) == false:
		return -1
	return int(terrain_data.get("move_cost", 1))

func _get_runtime_terrain_data(pos: Vector2i) -> Dictionary:
...
func _can_step_on(tile_map: TileMap, pos: Vector2i, destination: Vector2i) -> bool:
	...
	if pos == destination:
		return true
	var unit = _get_runtime_unit_at(pos)
	return unit == null

func _get_runtime_unit_at(pos: Vector2i):
