...
func add_effect(unit: UnitActor, effect_type: String, duration: int) -> void:
...
func remove_effect(unit: UnitActor, effect_type: String) -> void:
	if not unit.runtime_state:
		return
...
func tick_all(units: Array[UnitActor]) -> void:
	for unit in units:
		...
		for effect in effects:
			var dur := effect.get("duration", 1) - 1
			if dur > 0:
				effect["duration"] = dur
...
func has_effect(unit: UnitActor, effect_type: String) -> bool:
	if not unit.runtime_state:
		return false
...
func _apply_poison(unit: UnitActor) -> void:
	var dmg := max(1, unit.runtime_state.max_hp / 10)
